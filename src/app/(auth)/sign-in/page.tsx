...
export default function SignInPage() {
  ...
  const onSubmit = async (data: SignInFormData) => {
    ...
    try {
      ...
      if (error) {
        setServerError(error.message.includes('Invalid') ? 'That password does not match your account.' : 'Something went wrong. Please try again.')
        return
      }

      const email = authData.user?.email?.toLowerCase() || ''

      // Check if invited admin (not in hardcoded list)
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div style={{display:'flex', flexDirection:'column', gap:14}}>
          ...
          <button type="submit" disabled={isLoading}
            style={{width:'100%', height:48, background:'#7C3AED', color:'white', border:'none', borderRadius:12, fontSize:15, fontWeight:600, cursor:'pointer', opacity:isLoading?0.7:1, display:'flex', alignItems:'center', justifyContent:'center', gap:8}}>
      ...
      <div style={{display:'flex', flexDirection:'column', gap:10}}>
        <button onClick={handleGoogle}
          style={{width:'100%', height:44, background:'white', border:'1px solid #E5E7EB', borderRadius:10, fontSize:14, color:'#374151', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:10, fontWeight:500}}>
