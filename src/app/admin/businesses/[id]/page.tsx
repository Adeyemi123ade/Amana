...
export default async function BusinessDetailPage({ params }: { params: { id: string } }) {
  ...
  const { data: b } = await db.from('workspaces').select('*').eq('id', params.id).maybeSingle()
  if (!b) notFound()
  ...
  const [
    ...
    db.from('invoices').select('*', { count:'exact', head:true }).eq('workspace_id', params.id),
    db.from('customers').select('*', { count:'exact', head:true }).eq('workspace_id', params.id),
    db.from('appointments').select('*', { count:'exact', head:true }).eq('workspace_id', params.id),
    db.from('invoices').select('invoice_number,total_amount,status,created_at').eq('workspace_id', params.id).order('created_at', { ascending:false }).limit(5),
  ])
