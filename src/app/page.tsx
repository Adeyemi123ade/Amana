...
export default function LandingPage() {
  return (
    <main className="min-h-screen flex flex-col" style={{background: '#111827'}}>
      {/* Topbar */}
      <nav className="flex items-center justify-between px-5 py-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-bold text-white">ROS</span>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-5 py-10">
        ...
        <div className="mt-8 flex flex-col gap-3">
          <Link href="/sign-up" className="flex items-center justify-center rounded-xl bg-[#7C3AED] px-6 py-3.5 text-base font-semibold text-white">
            Get Started Free
          </Link>
          <Link href="/sign-in" className="flex items-center justify-center rounded-xl border border-white/20 px-6 py-3.5 text-base font-semibold text-white">
            Watch Demo
          </Link>
        </div>
        ...
        <div className="mt-8 flex items-center gap-3">
          <div className="flex -space-x-2">
            {['#7C3AED','#6D28D9','#5B21B6','#4C1D95'].map((c, i) => (
              <div key={i} className="h-8 w-8 rounded-full border-2 border-[#111827] flex items-center justify-center text-xs font-bold text-white" style={{background: c}}>
