...
-- 1. Add columns to reminder_logs
alter table public.reminder_logs
  add column if not exists workspace_id uuid references public.workspaces(id) on delete cascade,
  add column if not exists recipient_email text,
  add column if not exists error_message text;

-- 2. RLS for reminder_logs (drop first to avoid duplicate error)
drop policy if exists "reminder_logs_workspace" on public.reminder_logs;
create policy "reminder_logs_workspace" on public.reminder_logs
  for all using (
    workspace_id in (
...
-- 3. Seed default automation rules for existing workspaces
...
where not exists (
  ...
  where ar.workspace_id = w.id
    and ar.type = rules.type
    and ar.trigger_days = rules.trigger_days
);

-- 4. Ensure workspace slugs are set
...
drop policy if exists "booking_owner" on public.booking_requests;
...
-- 6. Public read on workspaces (safe to re-run)
drop policy if exists "ws_public_read" on public.workspaces;
