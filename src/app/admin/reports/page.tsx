...
export default async function AdminReportsPage() {
  ...
  const [
    ...
    db.from('kyc_submissions').select('*', { count: 'exact', head: true }).catch(() => ({ count: 0 })),
  ])
  const { data: paidPays } = await db.from('payments').select('amount').eq('status', 'SUCCESS').catch(() => ({ data: [] }))
  ...
  const reports = [
    ...
    { label: 'Appointment Activity Report', desc: `${appointments || 0} appointments scheduled`,                link: '/admin/logs' },
    { label: 'Customer Activity Report',    desc: `${customers || 0} customers across all businesses`,         link: '/admin/users' },
