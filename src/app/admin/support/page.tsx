...
export default function AdminSupportPage() {
  ...
  const updateStatus = async (id: string, status: string) => {
    await fetch('/api/admin/support', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ id, status }) })
  ...
  const sc: Record<string,[string,string]> = {
    OPEN:['#DC2626','#FEF2F2'], IN_PROGRESS:['#D97706','#FFFBEB'],
    RESOLVED:['#16A34A','#F0FDF4'], CLOSED:['#6B7280','#F9FAFB'],
  }
  ...
  return (
    <div>
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ fontSize:13, color:'var(--admin-text-muted)' }}>{loading ? 'Loading...' : msgs.filter(m => m.status==='OPEN').length + ' open messages'}</p>
      </div>

      {sentMsg && (
        <div style={{ background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:10, padding:'10px 16px', marginBottom:16, fontSize:13, color:'#16A34A' }}>{sentMsg}</div>
      )}
      ...
      <div style={{ background:'var(--admin-card)', borderRadius:12, border:'1px solid var(--admin-card-border)', overflow:'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding:40, textAlign:'center', color:'var(--admin-text-muted)', fontSize:13 }}>No support messages</div>
        ) : msgs.map((m) => {
            ...
            <div key={m.id} style={{ padding:'16px 20px', borderBottom:'1px solid var(--admin-card-border)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8, flexWrap:'wrap', gap:8 }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                  ...
                  <p style={{ fontSize:11, color:'var(--admin-text-muted)' }}>{new Date(m.created_at).toLocaleDateString('en-NG',{day:'numeric',month:'short',year:'numeric'})}</p>
                </div>
              </div>
      ...
      {confirmEmail && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:20 }}>
          <div style={{ background:'var(--admin-card)', borderRadius:16, padding:28, maxWidth:380, width:'100%', textAlign:'center' }}>
      ...
      {replyOpen && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1001, padding:16 }}>
          <div style={{ background:'var(--admin-card)', borderRadius:16, width:'100%', maxWidth:560, maxHeight:'90vh', overflowY:'auto' }}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid var(--admin-card-border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              ...
              <button onClick={() => setReplyOpen(null)} style={{ background:'none', border:'none', fontSize:22, cursor:'pointer', color:'var(--admin-text-muted)' }}>x</button>
            </div>
