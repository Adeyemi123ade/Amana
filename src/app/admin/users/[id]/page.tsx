...
export default async function UserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    db.from('workspaces').select('*').eq('created_by', user.id).maybeSingle(),
  ])
  ...
  return (
    <div>
      ...
      <div style={{ background: 'var(--admin-card)', borderRadius: 14, border: '1px solid var(--admin-card-border)', padding: 24, marginBottom: 24, display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
      ...
      <div className="admin-two-col">
        <div style={{ background: 'var(--admin-card)', borderRadius: 12, border: '1px solid var(--admin-card-border)', padding: '20px 24px' }}>
          ...
          {field('Last Sign In', user.last_sign_in_at ? new Date(user.last_sign_in_at).toLocaleString('en-NG', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : null)}
            ...
            <>
              ...
              <div style={{ marginTop: 8 }}>
                <Link href={'/admin/businesses/' + ws.id}
                  style={{ fontSize: 13, fontWeight: 600, color: 'var(--admin-accent,#0E1A6E)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
