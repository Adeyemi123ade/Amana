...
export default async function AdminPaymentsPage() {
  ...
  const { data: pays } = await db.from('payments').select('*, workspaces(name)').order('created_at',{ascending:false}).limit(100)
    ...
    <div>
      ...
      <div style={{ background:'white', borderRadius:12, border:'1px solid #E2E8F0', overflow:'hidden' }}>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 160px 90px 90px 90px', gap:10, padding:'10px 16px', background:'#F8FAFC', borderBottom:'2px solid #E2E8F0' }}>
          {['Reference','Business','Customer Email','Amount','Channel','Status'].map(h=><p key={h} style={{ fontSize:10, fontWeight:700, color:'#64748B', textTransform:'uppercase', letterSpacing:.5 }}>{h}</p>)}
        </div>
        {(pays||[]).map((p:any)=>(
          <div key={p.id} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 160px 90px 90px 90px', gap:10, padding:'11px 16px', borderBottom:'1px solid #F8FAFC', alignItems:'center' }}>
            ...
            <p style={{ fontSize:12, color:'#374151', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{p.workspaces?.name||'—'}</p>
