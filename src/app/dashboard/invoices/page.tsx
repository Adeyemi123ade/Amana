...
export default async function InvoicesPage() {
  ...
  return (
    <div>
      {/* Single header row — ONE button only */}
      ...
      <div style={{background:'white',borderRadius:14,border:'1px solid #F3F4F6',overflow:'hidden'}}>
        {/* Search + filter tabs — NO duplicate button here */}
        ...
        {!invoices || invoices.length === 0 ? (
          <div style={{padding:'60px 20px',textAlign:'center'}}>
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#D1D5DB" strokeWidth="1.5" strokeLinecap="round" style={{margin:'0 auto 12px',display:'block'}}>
          ...
          <div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 160px 110px 90px',gap:12,padding:'10px 20px',background:'#F9FAFB',fontSize:11,fontWeight:700,color:'#9CA3AF',textTransform:'uppercase',letterSpacing:0.5}}>
            ...
            {invoices.map((inv: any) => {
              ...
              return (
                <Link key={inv.id} href={`/dashboard/invoices/${inv.id}`} style={{display:'grid',gridTemplateColumns:'1fr 160px 110px 90px',gap:12,padding:'14px 20px',borderTop:'1px solid #F9FAFB',textDecoration:'none',alignItems:'center'}}>
                  <div>
                  ...
                  <span style={{fontSize:11,fontWeight:600,color,background:bg,padding:'3px 10px',borderRadius:20,display:'inline-block'}}>{inv.status.charAt(0)+inv.status.slice(1).toLowerCase()}</span>
                </Link>
              )
            })}
          </div>
