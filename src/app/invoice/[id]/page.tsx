...
export default function PublicInvoicePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // No reference in URL — customer just opened the invoice normally
      if (!verifyRef) return
      ...
      try {
        ...
        if (data.success) {
          ...
          // Only show an error for genuine failures, not for cancellations
          if (!isAbandoned) {
            setPayError(
              'We could not confirm your payment. If you were charged, contact the business with reference: ' + verifyRef
            )
          }
          // For abandoned/cancelled: just show the pay button again silently
        }
      } catch {
        // Network error — silently reset. Webhook will handle it if they were charged.
      } finally {
  ...
  // Reset paying state when customer returns from Paystack
  // Only fires when we are not actively verifying (prevents race condition)
  useEffect(() => {
    const resetOnReturn = () => {
  ...
  if (verifying) return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#F9FAFB', padding: 24, fontFamily: 'system-ui,sans-serif' }}>
      ...
      <p style={{ fontSize: 17, fontWeight: 700, color: '#111827', marginBottom: 6 }}>Checking...</p>
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px' }}>
        ...
        <div style={{ background: 'white', borderRadius: 18, padding: 20, boxShadow: '0 2px 12px rgba(0,0,0,0.07)', marginBottom: 14 }}>
          ...
          <button
            ...
            style={{
              width: '100%', height: 56,
              background: paying ? '#6D28D9cc' : 'linear-gradient(135deg,#7C3AED,#6D28D9)',
          ...
          >
            {paying ? (
              <>
                <span style={{ width: 20, height: 20, border: '2.5px solid white', borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'spin .7s linear infinite' }} />
